...
export default function ExpenseModal(props) {
  ...
  useEffect(() => {
    const parsedDate = new Date(props.initialDate);
    if (!isNaN(parsedDate)) {
      setDate(parsedDate);
    }
    setCategory(props.initialCategory || "");
    setAmount(props.initialAmount ? props.initialAmount.toString() : "");
...
const styles = StyleSheet.create({
  ...
  cancelText: { color: "#ff3333", textAlign: "center", fontSize: 15 },
  menuOverlay: {
    flex: 1,
    backgroundColor: "rgba(0,0,0,0.4)",
    justifyContent: "center",
